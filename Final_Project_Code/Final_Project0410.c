/* Final version of the MECH 458 
   Heng-Lai Wei & Zhuo Li
...
//volatile unsigned char dutyCycle = 0xb4; 	// set PWM = 60%/9a 70%/b4 80%/cd
...
void stepperRotate(int steps, int direction) {
	...
	uint8_t maxdelay = 16;
	uint8_t mindelay = 6;//5ms corresponds to 200 steps per second; or 1 revolution per second,
  //7 is the minimum with no error
	int delay = maxdelay;
	int i;
	...
	for(i=0;i<steps;i++){
	  ...
	  if (((steps-i)<10) && (delay<=maxdelay)) delay += 1;//deceleration
	}
} 
...
ISR(ADC_vect){					// ADC interrupt for reflecness conversion , PF1
	...
	if ((PIND & 0x04) == 4){
	  ...
	  //	PORTC = ADC_min; // calibrate the reflective value of different objects
